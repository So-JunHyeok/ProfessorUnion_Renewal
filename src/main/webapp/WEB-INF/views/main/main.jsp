...
<script src="http://code.jquery.com/jquery.min.js"></script>
  ...
  <script>
  	...
  	function tabclick3(){
  	  $('.activity-title').css("border-top" , "none")
  	  $('.activity-title').css("border-left" , "none")
  	  $('.activity-title').css("border-right" , "none")
  	  $('.activity-title').css("border-bottom" , "2px solid #23539b")
  	  
  	  $('.news-title').css("border-top" , "2px solid #23539b")
  	  $('.news-title').css("border-left" , "2px solid #23539b")
  	  $('.news-title').css("border-right" , "2px solid #23539b")
  	  $('.news-title').css("border-bottom" , "none")
  	  $('.news-title').css("border-top-left-radius" , "10px")
  	  $('.news-title').css("border-top-right-radius" , "10px")
  	  
  	  $(".activity-middle-two").show()
  	  $(".activity-middle").hide()
  	  $(".activity-plus a").attr("href","board_list.nhn?boardName=news")
    	}
    	
    	function tabclick4(){
   	  $('.news-title').css("border-top" , "none")
  	  $('.news-title').css("border-left" , "none")
  	  $('.news-title').css("border-right" , "none")
  	  $('.news-title').css("border-bottom" , "none")
  	  $('.news-title').css("border-top-left-radius" , "none")
  	  $('.news-title').css("border-top-right-radius" , "none")
  	  $('.news-title').css("border-bottom" , "2px solid #23539b")
    	  
    	  $('.activity-title').css("border-top" , "2px solid #23539b")
    	  $('.activity-title').css("border-left" , "2px solid #23539b")
    	  $('.activity-title').css("border-right" , "2px solid #23539b")
    	  $('.activity-title').css("border-bottom" , "none")
    	  $('.activity-title').css("border-top-left-radius" , "10px")
    	  $('.activity-title').css("border-top-right-radius" , "10px")
    	  
  	  $(".activity-middle").show()
  	  $(".activity-middle-two").hide()
      	}
      ...
      <div class="content-board">
        <div class="board">
          ...
          <div class="activity-news">
            <div class="activity-top">
              <div class="activity-title" onclick="tabclick4()">
                <a>언론보도</a>
              </div>
              <div class="news-title" onclick="tabclick3()">
                <a>소식지</a>
              </div>
              <div class="activity-fake"></div>
              <div class="activity-plus">
                <a href="board_list.nhn?boardName=press">더보기</a>
                <i class="fas fa-plus"></i>
              </div>
            </div>
              			...
              			<c:choose>
              				 <c:when test="${!empty mainPress[n]}">
              				 	<c:if test="${fn:length(mainPress[n].wr_subject) > 30}">
              		 				<li><a href="board_cont.nhn?wr_id=${mainPress[n].wr_id }&boardName=press"><i class="fas fa-angle-right"></i>${fn:substring(mainPress[n].wr_subject,0,30)}...</a></li>
              		 			</c:if>
              		 			<c:if test="${fn:length(mainPress[n].wr_subject) < 30}">
              		 				<li><a href="board_cont.nhn?wr_id=${mainPress[n].wr_id }&boardName=press"><i class="fas fa-angle-right"></i>${mainPress[n].wr_subject}</a></li>
              		 			</c:if>
              			 	</c:when>
              			 	<c:otherwise>
              		 			<li><a href="#"><i class="fas fa-angle-right"></i>게시글이 없습니다</a></li>
            ...
            <div class="activity-middle-two" style="display:none">
              <ul>
               <c:forEach var="n" begin="0" end="3" step="1">
              			<c:choose>
              				 <c:when test="${!empty mainNews[n]}">
              				 	<c:if test="${fn:length(mainNews[n].wr_subject) > 30}">
              		 				<li><a href="board_cont.nhn?wr_id=${mainNews[n].wr_id }&boardName=news"><i class="fas fa-angle-right"></i>${fn:substring(mainNews[n].wr_subject,0,30)}...</a></li>
              		 			</c:if>
              		 			<c:if test="${fn:length(mainNews[n].wr_subject) < 30}">
              		 				<li><a href="board_cont.nhn?wr_id=${mainNews[n].wr_id }&boardName=news"><i class="fas fa-angle-right"></i>${mainNews[n].wr_subject}</a></li>
              		 			</c:if>
              			 	</c:when>
              			 	<c:otherwise>
              		 			<li><a href="#"><i class="fas fa-angle-right"></i>게시글이 없습니다</a></li>
              			...
              			<c:choose>
              				 <c:when test="${!empty mainStatement[n]}">
              				 	<c:if test="${fn:length(mainStatement[n].wr_subject) > 30}">
              		 				<li><a href="board_cont.nhn?wr_id=${mainStatement[n].wr_id }&boardName=statement"><i class="fas fa-angle-right"></i>${fn:substring(mainStatement[n].wr_subject,0,30)}...</a></li>
              		 			</c:if>
              		 			<c:if test="${fn:length(mainStatement[n].wr_subject) < 30}">
          ...
          <div class="photo">
            ...
            <c:if test="${sessionID == null}">
            	<div style="text-align: center; margin-top: 20%;">로그인 후 이용하실 수 있습니다.</div>
            </c:if>
            <c:if test="${sessionID != null}">
                <div class="photo-img">
        ...
        <div class="slide-board">
	      ...
	      <img class="mySlides" onclick="PrImg1()" src="<c:url value='resources/community/imageUpload/${PrImg1}'/>">
           ...
           <div class="facebook" style="width:100%; height: 35px;margin-top: 5px;">
           <span style="width: 30px; height: 35px;"><a href="https://www.facebook.com/kpu.page.official" style="width: 100%; height: 35px;"><img alt="" src="resources/img/main/facebook1.jpg" width="100%;" height="35px;"></a></span>
           </div>
              			...
              			<c:choose>
              				 <c:when test="${!empty mainStatement[n]}">
              				 	<c:if test="${fn:length(mainStatement[n].wr_subject) > 24}">
              		 				<li><a href="board_cont.nhn?wr_id=${mainStatement[n].wr_id }&boardName=statement"><i class="fas fa-angle-right"></i>${fn:substring(mainStatement[n].wr_subject,0,24)}...</a></li>
              		 			</c:if>
              		 			<c:if test="${fn:length(mainStatement[n].wr_subject) < 24}">
              		 				<li><a href="board_cont.nhn?wr_id=${mainStatement[n].wr_id }&boardName=statement"><i class="fas fa-angle-right"></i>${mainStatement[n].wr_subject}</a></li>
              		 			</c:if>
