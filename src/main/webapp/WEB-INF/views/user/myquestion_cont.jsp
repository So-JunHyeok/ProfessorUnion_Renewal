...
<script>
		function commentReple(e){
			...
			var str = "";
			str += "<form action='quetion_comment' method='post' class='commentform'>";
			str += "<textarea name='wr_content' rows='8' cols='120' style='border-radius: 10px;'></textarea>";
        ...
        <div class="professor-contents">
          ...
          <div class="professor-center">
            <div class="title">
              ...
              <div class="title-2">
                <span class="span-1"><a href="question_list?boardName=${boardName}">목록</a></span>
                <span class="span-1"><a href="replyBoardWrite.nhn?boardName=${boardName }&wr_id=${ques.wr_id}&wr_parent=">답변</a></span>
              </div>
            </div>
            <div class="view-content">
              <div class="view-content-top">
                <div class="view-content-day">
                  <span>작성일 :</span>
                  <span>${ques.wr_datetime }</span>
                </div>
                <div class="view-content-writer">
                  <span>작성자 :</span>
                  <span>${ques.mb_id }</span>
                </div>
              </div>
              <div class="view-content-file">
                ...
                <div class="view-content-filelist">
                  ...
                  <ul>
                    <li><span class="span-7"><i class="fas fa-arrow-alt-circle-down"></i><a type="mime-type" href="downloadaction?filename=${f.bf_source }&wr_id=${wr_id}&boardName=${boardName}">${f.bf_source }</a></span></li>
                  </ul>
                    </c:forEach> --%>
                </div>
                    </c:if>
...
%> 
			...
			${fn:replace(ques.wr_content, crcn, br)}
				<%-- <c:if test="${'photo' eq boardName }"> --%>
                  <c:forEach var="f" items="${files}">
					<img class="photocont" alt="" src="resources/community/imageUpload/${f.bf_source }">
					</c:forEach>
				<%-- </c:if> --%>
              </div>
              <div class="view-content-bottom">
                <div class="view-content-bottom-link">
                  ...
                  <c:if test="${'admin' eq mb_id || mb_id eq board.mb_id}">
                  <span class="span-1"><a href="board_modify.nhn?wr_id=${ques.wr_id}&boardName=${boardName }">수정</a></span>
                  <span class="span-1"><a href="board_delete.nhn?wr_id=${ques.wr_id}&boardName=${boardName }" onclick="return confirm('한번 삭제 된 게시물 복구 할 방법이 없습니다.\n\n정말 삭제 하시겠습니까?');">삭제</a></span>
                  </c:if>
                  	<input type="hidden" name="wr_id" value="${ques.wr_id}">
                  	<input type="hidden" name="boardName" value="${boardName}">
                 	<input type="hidden" name="wr_subject" value="${ques.wr_subject}">
                  	<span class="span-1"><a href="#" onclick="javascript:document.myForm.submit();">스크랩 </a></span>
                  <span class="span-1"><a href="#">트랩백</a></span>
                  </form>
			...
			<form action="board_notice_comment" method="post" id="commentform">
              <div class="view-content-comment">
                <textarea name="wr_content" rows="8" cols="120" style="border-radius: 10px;"></textarea>
                <input type="submit" name="button" style="border-radius:10px; width:100px; height:80px;margin-left:20px; background:white; margin-top:20px;" value="댓글입력"></button>
